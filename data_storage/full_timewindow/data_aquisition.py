...
def add_to_vector(data, vector, first_timestamp, dic_offset):
    ...
    return np.append(vector, [new_data], axis=0), first_timestamp
...
def save_trainnning_data(data):
    ...
    for file in files:
        if file.find("raw_learning_data.") != -1:
            ...
            data = np.append(prev_data_array, [data], axis=2)
            file_exists = True
    ...
    if file_exists is False:
        ...
        print(data_aux)
        data_aux[:, :, 0] = data
        np.save(path + "/raw_learning_data.npy", data_aux)
    else:
...
if __name__ == '__main__':
    ...
    pos = f'time: {datetime.now().strftime("%d/%m/%Y %H:%M:%S")}, ' \
          f'number_experiments: {vector_data.shape[0]}, ' \
