...
if __name__ == '__main__':
    ...
    for n in progressbar(range(n_times), redirect_stdout=True):
        ...
        for n in progressbar(range(x_test.shape[0]), redirect_stdout=True):
            decoded_seq, seq = decode_sequence(x_test[n][np.newaxis, :, :], start_number, neurons, n_labels, e_inputs,
    ...
    with open(ROOT_DIR + "/neural_networks/recurrent/seq2seq_n_times/training_testing_n_times_seq2seq.json", "w") as wf:
