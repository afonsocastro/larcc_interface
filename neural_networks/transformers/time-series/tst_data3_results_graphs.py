...
if __name__ == '__main__':
    ...
    pred_tst = load('ts_transformer_data3_pred.npy')
    ...
    for n in range(0, len(all_true)):
        ...
        pull_tst, push_tst, shake_tst, twist_tst = value_for_array(pred, time_steps-sliding_window+1)

        df11 = pd.DataFrame({'timestep': cnn_times, 'pull_tst': pull_tst})
        df22 = pd.DataFrame({'timestep': cnn_times, 'push_tst': push_tst})
        df33 = pd.DataFrame({'timestep': cnn_times, 'shake_tst': shake_tst})
        df44 = pd.DataFrame({'timestep': cnn_times, 'twist_tst': twist_tst})
