...
if __name__ == '__main__':
    ...
    model.summary()
    plot_model(model, to_file="transformer_v3_0.png", show_shapes=True)

    # callbacks = [keras.callbacks.EarlyStopping(monitor='val_loss', patience=40, restore_best_weights=True)]

    fit_history = model.fit(x_train, y_train, shuffle=True, validation_split=validation_split, epochs=200, batch_size=32)
                            # callbacks=callbacks)
    model.save("transformer_v3_0.keras")
    fig = plt.figure()
